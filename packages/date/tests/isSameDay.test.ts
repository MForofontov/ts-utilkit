...
describe('isSameDay', () => {
  ...
  it('3. should return false for consecutive days', () => {
    expect(isSameDay(new Date('2025-03-15'), new Date('2025-03-16'))).toBe(false);
  });

  it('4. should return false for the same day-of-month in different months', () => {
    expect(isSameDay(new Date('2025-01-15'), new Date('2025-02-15'))).toBe(false);
  });

  it('5. should return false for the same month/day in different years', () => {
    expect(isSameDay(new Date('2024-06-15'), new Date('2025-06-15'))).toBe(false);
  });
  ...
  it('8. should return false for the last day of a month vs the first of the next', () => {
    expect(isSameDay(new Date('2025-01-31'), new Date('2025-02-01'))).toBe(false);
  });

  it('9. should return false for Dec 31 vs Jan 1 of the next year', () => {
    expect(isSameDay(new Date('2024-12-31'), new Date('2025-01-01'))).toBe(false);
  });
  ...
  it('11. should throw Error for invalid date1', () => {
    expect(() => isSameDay(new Date('invalid'), new Date('2025-01-01'))).toThrow('Invalid date');
  });

  it('12. should throw Error for invalid date2', () => {
    expect(() => isSameDay(new Date('2025-01-01'), new Date('invalid'))).toThrow('Invalid date');
  });
});
