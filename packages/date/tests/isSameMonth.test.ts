...
describe('isSameMonth', () => {
  ...
  it('3. should return false for the same month in different years', () => {
    expect(isSameMonth(new Date('2024-03-15'), new Date('2025-03-15'))).toBe(false);
  });

  it('4. should return false for adjacent months in the same year', () => {
    expect(isSameMonth(new Date('2025-03-15'), new Date('2025-04-15'))).toBe(false);
  });

  it('5. should return false for the first and last months of the year', () => {
    expect(isSameMonth(new Date('2025-01-01'), new Date('2025-12-31'))).toBe(false);
  });
  ...
  it('8. should return false for the same month number in different years', () => {
    expect(isSameMonth(new Date('2000-12-15'), new Date('2025-12-15'))).toBe(false);
  });

  it('9. should throw Error for invalid date1', () => {
    expect(() => isSameMonth(new Date('invalid'), new Date('2025-01-01'))).toThrow('Invalid date');
  });

  it('10. should throw Error for invalid date2', () => {
    expect(() => isSameMonth(new Date('2025-01-01'), new Date('invalid'))).toThrow('Invalid date');
  });
});
